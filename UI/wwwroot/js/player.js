(() => {
    ...
    async function loadLookups() {
        if (!isStaff) return;
        const r = await fetch('/api/players/lookups');
        if (!r.ok) return;
    ...
    // ---------- таблица ----------
    const columns = [
        { title: 'Фамилия', field: 'lastName', minWidth: 120 },
        { title: 'Имя', field: 'firstName', minWidth: 120 },
        { title: 'Дата рождения', field: 'birthDate', width: 140, formatter: c => fmtDate(c.getValue()) },
        { title: 'Возраст', field: 'age', width: 100, headerSort: false, hozAlign: 'center' },
        { title: 'Группа', field: 'groupName', width: 110 },
        { title: 'Родитель', field: 'parentName', minWidth: 160 },
        {
            title: 'Медсправка до', field: 'medicalCertificateUntil', width: 150,
            formatter: c => {
                const v = c.getValue();
                if (!v) return '<span class="text-muted">нет</span>';
                const cls = v < today ? 'text-danger fw-bold' : '';
                return `<span class="${cls}">${fmtDate(v)}</span>`;
            }
        },
        { title: 'Активен', field: 'isActive', width: 100, hozAlign: 'center', formatter: 'tickCross' }
    ];
