...
(() => {
    ...
    let coaches = [], activeGroups = [], playersWithOutGroup = [], current = null; pl = null;
    ...
    async function loadLookups() {
        ...
        const pwg = await (await fetch('/api/groups/playerswithoutgroup')).json();
        playersWithOutGroup = pwg;
        console.log(playersWithOutGroup);
    }
    ...
    function fillPlayers(select, exceptId, firstLabel) {
        select.length = 0;
        select.add(new Option(firstLabel, 0));
        for (const g of playersWithOutGroup) if (g.id !== exceptId) select.add(new Option(`${g.firstName} ${g.lastName}`, g.id));
    }

    $('inTarget').addEventListener('change', async (e) => {
        ...
        if (e.target.value != 0) {
            ...
            // const r = await json(`/api/groups/${current.id}/players/move`, 'POST', { playerIds: e.target.value, targetGroupId: target });
    ...
    $('btnIn').addEventListener('click', async () => {
        ...
        console.log(current);
        console.log(pl);
        const label = `Перевести ${$('inTarget').selectedOptions[0].text} в группу ${current.name}?`;
        if (!confirm(label)) return;
        const r = await json(`/api/groups/${current.id}/players/add`, 'POST', { playerIds: arr, targetGroupId: current.id });
        if (!r.ok) return alert(await errorOf(r));
        await loadLookups(); table.setData(); showRoster(current);
    });
    ...
    const roster = new Tabulator('#rosterTable', {
        ...
        height: '420px',
        selectable: true,
        placeholder: 'В группе нет игроков',
        rowFormatter: row => {
            const d = row.getData();
            if (!d.medicalValid) row.getElement().classList.add('table-warning');
        },
        columns: [
    ...
    function showRoster(g) {
        ...
        fillPlayers($('inTarget'), -1, '- выберите игрока (без группы) - ');
